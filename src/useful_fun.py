...
def plot_distribution(X, Y, mean, var, pos, labels, legend=True):
    ...
    c = 1.96 * np.sqrt(var)
    plt.fill_between(X[:,0], (mean - c)[:,0], (mean + c)[:,0], alpha=0.2, edgecolor='gray', facecolor='C0', label='95% CI')
...
def plot_sliding_window(x_train, x_test, y_train, y_test, mean_train, mean_test, var_train, var_test, pos, labels, iteration):
    ...
    plt.fill_between(x_train[:,0], (mean_train - c_train)[:,0], (mean_train + c_train)[:,0], alpha=0.4, edgecolor='gray', facecolor='C0', label='95% CI for train')
